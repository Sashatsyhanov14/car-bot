...
const AdminExcursions: React.FC<{ t?: any }> = () => {
    ...
    const [formData, setFormData] = useState({
        city: '', title: '', description: '', price_rub: 0, duration: '', image_url: '', sort_number: 1, is_active: true
    });
    ...
    const handleSave = async () => {
        ...
        setIsEditing(null);
        setFormData({ city: '', title: '', description: '', price_rub: 0, duration: '', image_url: '', sort_number: 1, is_active: true });
        fetchExcursions();
    };
            ...
            <div className="bg-[#1a1a1d] p-6 rounded-3xl border border-white/5 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                        ...
                        <input value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} className="w-full bg-black/30 border border-white/10 rounded-xl p-3 text-sm" placeholder="Обзорная" />
                    </div>
                    <div className="space-y-1 md:col-span-2">
                        <label className="text-[10px] font-bold text-slate-500 uppercase ml-2">Описание</label>
                        <textarea value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} className="w-full bg-black/30 border border-white/10 rounded-xl p-3 text-sm h-24" />
                    </div>
                    <div className="space-y-1">
                    ...
                    <div className="space-y-1">
                        ...
                        <input value={formData.duration} onChange={e => setFormData({ ...formData, duration: e.target.value })} className="w-full bg-black/30 border border-white/10 rounded-xl p-3 text-sm" />
                    </div>
                </div>
                ...
                {excursions.map(ex => (
                    <div key={ex.id} className="bg-[#1a1a1d] p-4 rounded-3xl border border-white/5 flex gap-4 items-center">
                        <div className="w-16 h-16 bg-black/40 rounded-2xl flex items-center justify-center overflow-hidden flex-shrink-0">
                            {ex.image_url ? <img src={ex.image_url} alt="" className="object-cover w-full h-full" /> : <span className="material-symbols-outlined text-slate-600">map</span>}
                        </div>
                        <div className="flex-1 min-w-0">
                            <h4 className="font-bold truncate text-slate-200">{ex.title}</h4>
                            <p className="text-xs text-slate-400">{ex.price_rub}₽</p>
                        </div>
